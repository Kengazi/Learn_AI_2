...
Use Open CV to caputure constant video feed from camera and mask certien color object that allows the camera to put boxes around a samecolor of the mask.
...
while True:
    ...
    # frame is essentially the image being loop the produces a video feed
    _, frame = vid.read()
    
    # Convert BGR(Blue, Green, Red) image to HSV(Hue, Saturation, Value)
    ...
    # the mask for the color blue - green - cyan
    #bright_blue = np.array([70,50,50])
    #dark_blue = np.array([100,255,255])
    #blue_mask = cv2.inRange(hsv_frame, dark_blue,bright_blue)
...
Run the code. It should be a basic webcam forward facing webcam.

Uncomment everything that is in the script that is code

Try using the red mask on a red object.

Try using the yellow mask on a yellow object.

Try using the blue mask on a blue object.
