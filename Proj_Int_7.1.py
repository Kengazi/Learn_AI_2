...
ANN overview Youtube video link: https://www.youtube.com/watch?v=aircAruvnKk&t=644s :: Watch it

ANN scikit learn Youtube video link: https://www.youtube.com/watch?v=X8SPO875mQY&t=429s :: Watch it
...
model = MLPClassifier(activation='tanh',solver='sgd', alpha=1e-4,hidden_layer_sizes=(90,90),
...
with open(ANN,'wb') as file:
    ANN_model = pickle.dump(ANN,file)
...
# Predicting    
Pred = model.predict([[1.,20.,2.,1.,30.]])
Pred = Pred[0]
if Pred == 1:
    print('Passenger lived')
else:
...
Predict if a passenger lived or died.
...
How is this model score compared to the other models on the difficult titanic dataset?
...
BONUS use the K-FOLD with k = 5

BONUS use the one hot encoder on gender and add it as a catigory to the neural network

BONUS BONUS Get over a 90% Model score
