...
The goal of this assignment is to get you comfortable with doing working with data and files in python using pandas.
...
# Calulating our own list of NNs from the datasets given.
...
"""
Plotting to visually compare Calculated points above with the orginally given points. To confirm the quility of our data.

"""

# HR vs NN
plt.scatter(HR_A,NN_A,color='red',label = 'Calc A')     # Plot Calc Data
plt.scatter(HR_K,NN_K,color='blue',label = 'Calc K')    # Plot Calc Data
plt.scatter(HR_A,NN,color='green',label = 'Given A')    # Plot Givin Data
plt.scatter(HR_K,NN,color='black',label = 'Given K')    # Plot Givin Data
...
If you were told that these are a random readings from 4 different heart moniters can you tell
which are stimulated events and which are not from lookin at the graph?

Which heart reader is the highest quality ? And which is the lowest quality?

The red dots are the actual heart reader values. Do you think they were all accurate?

Now use the csv file HR4.csv with 460 HR values in Beats Per 10 Sec (not bpm) 
to make a graph of the NN above and answer the same questions as above.
