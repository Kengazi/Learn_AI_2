...
xc = np.linspace(0,1000,1001)                                    # using linspace to make a set between 0 and 10 with 1001 digits
...
"""
 ...
 BONUS Which one decays to zero the fastest (put on same plot using regular plot (plt.plot))
 
 D = 10.234
 G = .54
 T = t/G
 t = 0 to 10 
 Da(t) = D * (1/2)*(t/T(t)) * -t + 10
 Db(t) = D * np.exp(-2/T(t)) - (1/2) * t
 Dc(t) = D * np.log(4 - np.exp(G*t)) + 10
