...
# Data from June xlsx 
Temp = best_df.values[:,0]                   # Degrees F     
Humidity = best_df.values[:,1]               # in percent
Wind = best_df.values[:,2]                   # Wind speed mph
...
if pred > .5:
    ...
    print("Pecertage it will Rain {:.4}%".format(100-float(pred)))
    
    
"""
Your Turn

Change the linear regression model to, model = linear_model.LogisticRegression()

Does this improve the model ? Why? (What quatilies of logistic knowing the dataset) and print new log model score.

Try another model. What is the model and what score do you get.

Please look at all the pandas changes of dataset and comment your opinon of which is best to predict this data

Compare (model score) the method I used for missing data with the 2 other methods comment your opinon and new model score
