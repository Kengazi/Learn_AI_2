...
The goal of this assignment is to get you comfortable with doing working with files in python using pandas, and looking at real data

Pandas is a very versitile module in python that allows you make, upload, and download data and files with ease.
...
"""
A and K are essentually the same patient at different testing times
...
NN = df.values[:,5]                 # The difference in NN values from the current and the previous
SDNN = df.values[:,6]               # Standard Dev of the NN values
pNN20= df.values[:,7]               # Percent of the total number of NN's the are bigger the 20
pNN50= df.values[:,8]               # Percent of the total number of NN's the are bigger the 50
Stim_real = df.values[:,9]          # Stimulas events from given data
...
HR_A = np.array(HR_A)   
HR_A = pd.DataFrame({'HR':HR_A})

IBI_A = np.array(IBI_A)   
IBI_A = pd.DataFrame({'IBI':IBI_A})
...
NN = np.array(NN)   
NN = pd.DataFrame({'NN':NN})

# Varables from NN DataFrame the max, standard dev, mean, min, sum

# Max  
Max = NN.max() # makes a list that needs to be sliced
Max = Max[0] # this is the sliceing // the bracked zero refers to the first digit in list
...
# Finding if our dataset for SDNN is consistent by performing our own Stand dev on our given NN data
...
"""
The reason their is one calculated value is between we found the standard deviation of all the values instead of some interval like the given data
"""
...
From looking at the graphs did this person has any stimulas, or stimilus-like events?(A stimulas would be a very big outlier)
